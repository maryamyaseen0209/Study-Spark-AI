...
export default function QuizPanel({ role }) {
  ...
  return (
    <section id="quizzes" className="card mt-6 scroll-mt-24">
      ...
      {canManage && (
        <form onSubmit={generateQuiz} className="mt-5 rounded-2xl border border-white/10 bg-slate-950/50 p-4">
          <div className="flex items-center gap-2 text-lg font-black text-white"><FileUp className="h-5 w-5 text-spark-500" /> Generate from document</div>
      ...
      {editingQuiz && canManage && (
        <div className="mt-5 rounded-2xl border border-amber-400/30 bg-amber-400/10 p-5">
          <h3 className="text-xl font-black text-white">Teacher preview: {editingQuiz.title}</h3>
          <p className="mt-1 text-sm text-amber-100">This quiz is still a draft. Edit anything you need before publishing it to students.</p>
          ...
          <div className="mt-4 space-y-3">{editingQuiz.questions?.map((question, questionIndex) => <div key={questionIndex} className="rounded-2xl bg-black/20 p-4"><div className="grid gap-3 md:grid-cols-[1fr_150px]"><input value={question.text} onChange={(event) => updateQuestion(questionIndex, 'text', event.target.value)} className="input" /><select value={question.difficulty} onChange={(event) => updateQuestion(questionIndex, 'difficulty', event.target.value)} className="input"><option>easy</option><option>medium</option><option>hard</option></select></div><div className="mt-3 grid gap-2 md:grid-cols-2">{question.options.map((option, optionIndex) => <label key={optionIndex} className="flex items-center gap-2 rounded-xl bg-white/5 p-2"><input type="radio" checked={option.isCorrect} onChange={() => setCorrectOption(questionIndex, optionIndex)} /><input value={option.text} onChange={(event) => updateOption(questionIndex, optionIndex, event.target.value)} className="input py-2" /></label>)}</div><input value={question.explanation || ''} onChange={(event) => updateQuestion(questionIndex, 'explanation', event.target.value)} className="input mt-3" placeholder="Explanation shown after submission" /></div>)}</div>
      ...
      {activeQuiz && (() => {
        ...
        return (
        <div className="mt-5 rounded-2xl border border-spark-500/30 bg-spark-500/10 p-5">
          <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between"><div><h3 className="text-xl font-black text-white">Taking: {activeQuiz.title}</h3><p className="mt-1 text-sm text-slate-300">Question {currentQuestionIndex + 1} of {activeQuiz.questions.length} - {answeredCount} answered. Submit before the timer ends.</p></div><div className="inline-flex items-center gap-2 rounded-2xl bg-black/30 px-4 py-3 text-lg font-black text-white"><Clock3 className="h-5 w-5 text-cyan-300" /> {formatTime(timeLeft)}</div></div>
          <div className="mt-4 rounded-full bg-black/30 p-1"><div className="h-2 rounded-full bg-spark-500 transition-all" style={{ width: `${((currentQuestionIndex + 1) / activeQuiz.questions.length) * 100}%` }} /></div>
          <div className="mt-4 rounded-2xl bg-black/20 p-4"><p className="font-bold text-white">{currentQuestionIndex + 1}. {question.text}</p><div className="mt-3 grid gap-2">{question.options.map((option, optionIndex) => <label key={optionIndex} className="flex cursor-pointer items-center gap-3 rounded-xl bg-white/5 p-3 text-sm text-slate-200 transition hover:bg-white/10"><input type="radio" name={`question-${currentQuestionIndex}`} checked={answers[currentQuestionIndex] === optionIndex} onChange={() => setAnswers((current) => ({ ...current, [currentQuestionIndex]: optionIndex }))} />{option.text}</label>)}</div></div>
      ...
      {result && <div className="mt-5 rounded-2xl border border-emerald-400/30 bg-emerald-400/10 p-5"><h3 className="text-xl font-black text-white">Your marks: {result.attempt.correctCount}/{result.attempt.totalQuestions} correct - {result.attempt.score}% ({result.attempt.passed ? 'Passed' : 'Needs review'})</h3><div className="mt-4 space-y-3">{result.quiz.questions.map((question, index) => <div key={index} className="rounded-xl bg-black/20 p-3 text-sm"><p className="font-bold text-white">{question.text}</p><p className="mt-1 text-slate-300">Explanation: {question.explanation}</p></div>)}</div></div>}

      <div className="mt-6 grid gap-4 lg:grid-cols-2">
        {quizzes.map((quiz) => <article key={quiz._id} className="rounded-2xl border border-white/10 bg-slate-900/60 p-5"><div className="flex items-start justify-between gap-3"><div><h3 className="text-xl font-black text-white">{quiz.title} {!canManage && !quiz.latestAttempt && <span className="ml-2 rounded-full bg-blue-500 px-2 py-1 text-xs font-black text-white">New</span>}</h3><p className="mt-1 text-sm text-slate-400">{quiz.subject} - {quiz.classroom?.name || 'No classroom'} - {quiz.questions?.length || 0} questions - {quiz.durationMinutes} min{quiz.dueAt ? ` - due ${new Date(quiz.dueAt).toLocaleString()}` : ''}</p></div><span className={`rounded-full px-3 py-1 text-xs font-bold uppercase ${quiz.status === 'published' ? 'bg-emerald-400/10 text-emerald-300' : 'bg-amber-400/10 text-amber-300'}`}>{quiz.status}</span></div>{quiz.description && <p className="mt-3 text-sm leading-6 text-slate-300">{quiz.description}</p>}{quiz.latestAttempt && <p className="mt-3 rounded-xl bg-emerald-400/10 px-3 py-2 text-sm text-emerald-200">Your marks: {quiz.latestAttempt.score}%</p>}<div className="mt-4 flex flex-wrap gap-2">{canManage ? <><button disabled={busyId === quiz._id || quiz.status === 'published'} onClick={() => setEditingQuiz(quiz)} className="btn-secondary gap-2 px-3 py-2"><CheckCircle2 className="h-4 w-4" /> Preview/Edit</button><button disabled={busyId === quiz._id || quiz.status === 'published'} onClick={() => runQuizAction(quiz._id, () => api.patch(`/quizzes/${quiz._id}/publish`))} className="btn-primary gap-2 px-3 py-2"><Send className="h-4 w-4" /> Publish</button><button disabled={busyId === quiz._id} onClick={() => loadAnalytics(quiz._id)} className="btn-secondary gap-2 px-3 py-2"><BarChart3 className="h-4 w-4" /> Analytics</button><button disabled={busyId === quiz._id} onClick={() => runQuizAction(quiz._id, () => api.delete(`/quizzes/${quiz._id}`))} className="btn-secondary gap-2 px-3 py-2 text-rose-200"><Trash2 className="h-4 w-4" /> Delete</button></> : <button disabled={busyId === quiz._id || quiz.latestAttempt} onClick={() => startQuiz(quiz._id)} className="btn-primary gap-2 px-3 py-2"><Play className="h-4 w-4" /> {quiz.latestAttempt ? 'Submitted' : 'Start timed quiz'}</button>}</div>{analytics?.quizId === quiz._id && <div className="mt-4 grid gap-2 rounded-xl bg-black/20 p-3 text-sm text-slate-200 sm:grid-cols-3"><span>Submissions: <strong>{analytics.submissions}</strong></span><span>Average: <strong>{analytics.averageScore}%</strong></span><span>Passed: <strong>{analytics.passCount}</strong></span></div>}</article>)}
